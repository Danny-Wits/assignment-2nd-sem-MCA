/* ============================================
   app.js — MCA Department Website
   Vanilla JS + Supabase REST API
   ============================================ */
...
(function () {
  ...
  var SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InRhYnp6eXRkZWhrY3dwY25qZWZsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzY3ODY0MDcsImV4cCI6MjA5MjM2MjQwN30.-aQ_PprETXNWMBC07l6tNqdAvd4aNHQjxTzf7_CCw5M';
  ...
  if (student_list) {
    fetch(SUPABASE_URL + '/rest/v1/students?select=student_name,previous_degree&order=created_at.desc', {
      method: 'GET',
      headers: headers()
    })
      .then(function (res) {
      ...
      .then(function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          html +=
            ...
            '<span class="student_avatar rounded">' + initials + '</span>' +
            '<div>' +
            '<div class="fw-semibold">' + escape_html(s.student_name) + '</div>' +
            '<small class="text-muted">' + escape_html(s.previous_degree) + '</small>' +
            '</div>' +
            '</div>';
        }
        html += '</div>';
      ...
      .catch(function (err) {
        ...
        student_list.innerHTML =
          ...
          '<i class="ph ph-warning-circle"></i> ' + escape_html(err.message) +
          '</div>';
      });
  }
